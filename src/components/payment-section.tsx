...
export function PaymentSection({ invoice }: PaymentSectionProps) {
  ...
  const showCurrencyConversion = invoice.invoiceCurrency !== invoice.paymentCurrency;
        ...
        <div className="space-y-8">
          ...
          <div className="flex justify-center">
            <div className="flex items-center space-x-4">
              <div className={`flex items-center ${currentStep >= 1 ? 'text-black' : 'text-zinc-300'}`}>
                <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center
                  ${currentStep >= 1 ? 'border-black bg-zinc-50' : 'border-zinc-300'}`}>
                  <Wallet className="w-4 h-4" />
                </div>
                <span className="ml-2 font-medium">Connect Wallet</span>
              </div>
              <div className={`w-16 h-0.5 ${currentStep >= 2 ? 'bg-black' : 'bg-zinc-300'}`} />
              <div className={`flex items-center ${currentStep >= 2 ? 'text-black' : 'text-zinc-300'}`}>
                <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center
                  ${currentStep >= 2 ? 'border-black bg-zinc-50' : 'border-zinc-300'}`}>
            ...
            {currentStep === 2 && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-zinc-600">
                  Please confirm the payment amount and recipient address before proceeding.
                  {showCurrencyConversion && ` Payment will be processed in ${formatCurrencyLabel(invoice.paymentCurrency)}.`}
                </p>
                <Button
